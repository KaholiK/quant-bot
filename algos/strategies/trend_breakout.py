...
class TrendBreakoutStrategy:
    ...
    def calculate_signals(self,
                         ...
                         universe_data: dict[str, pd.DataFrame] | None = None) -> dict[str, Any]:
        """
    ...
    def _check_momentum_condition(self,
                                 symbol: str,
                                 price_data: pd.DataFrame,
                                 universe_data: dict[str, pd.DataFrame] | None) -> int:
        """
        ...
        try:
            ...
            if rank_percentile <= self.momentum_rank_pct:  # Top 30%
                return 1
            if rank_percentile >= (1.0 - self.momentum_rank_pct):  # Bottom 30%
                return -1
            return 0
    ...
    def _check_rsi_condition(self, features: pd.DataFrame) -> int:
        ...
        if rsi >= self.rsi_threshold:  # Overbought - avoid longs
            return -1
        if rsi <= (100 - self.rsi_threshold):  # Oversold - avoid shorts
            return 1
        return 0
    ...
    def _check_exit_conditions(self,
        ...
        if position_size > 0 and trend_signal == -1:  # Long position, trend turned down
            ...
            return True
        if position_size < 0 and trend_signal == 1:  # Short position, trend turned up
        ...
        if position_size > 0 and rsi_signal == -1:  # Long position, RSI overbought
            ...
            return True
        if position_size < 0 and rsi_signal == 1:  # Short position, RSI oversold
