...
def download_crypto(
    ...
    """
    Download crypto data and save to cache.
    
    Args:
        provider: Data provider (coingecko, cryptocompare)
    ...
    if provider == "coingecko":
        if not settings.COINGECKO_API_KEY:
            console.print("[yellow]⚠️  CoinGecko API key not set. Using public API (rate limited)[/yellow]")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Download crypto OHLCV data")
    parser.add_argument(
        "--provider",
        choices=["coingecko"],
        default="coingecko",
