...
from storage.db import EquityPoint, Fill, get_session
from ui.discord_bot.cogs.reporting import (
    create_equity_chart,
    create_error_embed,
    create_success_embed,
    create_trade_table_embed,
)
...
async def trades(interaction: discord.Interaction, limit: int = 20, symbol: str | None = None):
    """Show recent trades."""
    if not check_guild(interaction):
        await interaction.response.send_message(
            "❌ This command can only be used in the configured guild.",
