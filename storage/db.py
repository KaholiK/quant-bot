...
class Run(Base):
    ...
    universe: Mapped[str | None] = mapped_column(Text, nullable=True)  # CSV of symbols
    params: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # Run parameters
    kpis: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # Final KPIs
...
class Order(Base):
    ...
    run_id: Mapped[str | None] = mapped_column(String(36), nullable=True)  # FK to runs
...
def dump_run_kpis(run_id: str) -> dict[str, Any] | None:
    """
